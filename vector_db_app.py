...
# # Dummy data for K-Means (e.g., patient features: [age, temp, pain_level, fatigue_score])
# dummy_patient_data = np.array([
#     [30, 99.5, 7, 8], [25, 101.2, 6, 9], [35, 98.6, 2, 3], # Cluster 1 (acute)
#     [60, 98.0, 5, 6], [65, 97.8, 4, 5], [55, 98.2, 3, 4], # Cluster 2 (chronic/mild)
#     [10, 102.1, 9, 10], [8, 100.5, 8, 9], [12, 99.0, 5, 7]  # Cluster 3 (pediatric/severe)
# ])
...
#         file = request.files.get("image_file")
#         kmeans_input_str = request.form.get("kmeans_input", "")
#         # kmeans_input_str =kmeans_input_str + 1 ;
...
#                             if label == 'fracture' and conf > 0.5:
#                                 detections.append({
#                                     'box': {'x1': x1, 'y1': y1, 'x2': x2, 'y2': y2},
#                                     'name': label,
#                                     'confidence': conf
...
#                         fracture_summary = "Detected fractures: " + ", ".join(fracture_info_for_llm)
#                         image_analysis_results += f"<br><strong>{fracture_summary}</strong>"
#                         combined_query += f" {fracture_summary}"

#                 except Exception as e:
#                     image_analysis_results += f"<br>Error during YOLOv8 detection: {e}"
#             else:
#                 image_analysis_results += "<br>YOLOv8 model not loaded."
...
#             D, I = faiss_index.search(query_vector, 10)
#             threshold = 0.65
...
#             if image_analysis_results:
#                 clean_image_analysis = re.sub(r'<[^>]+>', '', image_analysis_results)
#                 prompt = f"Based on the image analysis: '{clean_image_analysis}'. And the user's query: '{user_query}'. Describe the medical implications, potential recovery, or general tips related to a bone fracture or the overall health concern mentioned."
#             else:
#                 prompt = user_query
...
#                     cluster_description = {
#                         0: "This patient profile aligns with an acute/severe symptom cluster.",
#                         1: "This patient profile aligns with a chronic/mild symptom cluster.",
#                         2: "This patient profile aligns with a pediatric/severe symptom cluster."
#                     }.get(cluster_id, "Unknown cluster type.")
#                     kmeans_cluster_result = f"Input belongs to cluster: {cluster_id + 2 }. {cluster_description}"
#                 else:
#                     kmeans_cluster_result = f"K-Means input requires {dummy_patient_data.shape[1]} comma-separated numbers (e.g., age, temp, pain_level, fatigue_score)."
#             except ValueError:
#                 kmeans_cluster_result = "Invalid K-Means input. Please enter comma-separated numbers."
